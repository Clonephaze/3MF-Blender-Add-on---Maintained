...
class Test3MFConsortiumSamples(Blender3mfTestCase):
    ...
    def _test_sample_file_roundtrip(self, filename, expected_elements=None):
        ...
        self.assertGreater(
            len(imported_objects), 0, f"No mesh objects imported from {filename}"
        )
        ...
        try:
            ...
            self.assertIsNotNone(
                exported_content, "Could not read exported model content"
            )
            ...
            for elem_name, original_count in original_counts.items():
                if original_count > 0:
                    self.assertGreaterEqual(
                        ...
                        original_count,
                        f"{elem_name} count decreased: "
                        f"original={original_count}, exported={exported_counts[elem_name]}",
                    )
        finally:
    ...
    def test_dodeca_chain_loop_color(self):
        ...
        self._test_sample_file_roundtrip(
            "dodeca_chain_loop_color.3mf", expected_elements=["colorgroup"]
        )

    def test_pyramid_vertexcolor(self):
        """Test vertex color (colorgroup) sample file."""
        self._test_sample_file_roundtrip(
            "pyramid_vertexcolor.3mf", expected_elements=["colorgroup"]
        )
    ...
    def test_multipletextures(self):
        ...
        """
        self._test_sample_file_roundtrip(
            "sphere_logo.3mf", expected_elements=["texture2d", "texture2dgroup"]
        )
    ...
    def test_sphere_logo_opc_texture_structure(self):
        ...
        try:
            ...
            with zipfile.ZipFile(export_path, "r") as archive:
                ...
                self.assertGreater(
                    len(texture_files), 0, "No texture files found in exported archive"
                )
